...
public class WaveformAnalyser{
    ...
    public static void main(String[] args){
        ...
        UI.setMouseListener(wav::doMouse);   
        //core
        UI.addButton("Read Data", wav::read);
        UI.addButton("Show Spread", wav::showSpread);
        UI.addButton("Increase region", wav::increaseRegion);
        UI.addButton("Decrease region", wav::decreaseRegion);
        //completion
        UI.addButton("Peaks", wav::highlightPeaks);
        UI.addButton("Distortion", wav::displayDistortion);
        UI.addButton("Delete", wav::deleteRegion);
        //challenge
        UI.addButton("Duplicate", wav::duplicateRegion);
        UI.addButton("Envelope", wav::displayEnvelope);
        UI.addButton("Save", wav::save);

        UI.addButton("Display", wav::display);
    ...
    public void read(){
        UI.clearPanes();
        String fname = UIFileChooser.open("Choose waveform file");
        ...
        UI.printMessage("Read " + waveform.size() + " data points from " + fname);
        this.regionStart = 0;
        this.regionEnd = waveform.size();
        this.display();
    ...
    public void display(){
        ...
        for(int i = 0; i<waveform.size()-1; i++){
            // n++;
            // UI.drawLine(GRAPH_LEFT+n, ZERO_LINE-waveform.get(i), GRAPH_RIGHT+n+1, ZERO_LINE-waveform.get(i+1));
            UI.printf("%f, %f\n", this.waveform.get(i), this.waveform.get(i+1));
        }
